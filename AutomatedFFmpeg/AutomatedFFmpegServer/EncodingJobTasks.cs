    ...
    public static class EncodingJobTasks
    {
        public static void BuildEncodingJob(EncodingJob job, CancellationToken cancellationToken)
        {
            ...
            if (cancellationToken.IsCancellationRequested)
            {
                ...
                // TODO: Log Cancel
                Console.WriteLine($"{nameof(BuildEncodingJob)} was cancelled for {job}");
                return;
            }
            ...
            try
            {
                string crop = GetCrop(job.SourceFullPath);
            }
            catch (Exception ex)
            {
                // TODO: Log Error
        ...
        private static string GetCrop(string sourceFullPath)
        {
            string crop = string.Empty;
            string ffprobeArgs = $"-i \"{sourceFullPath}\" -ss %s -t 00:02:00 -vf cropdetect -f null - 2>&1 | awk '/crop/ {{ print $NF }}' | tail -1";
