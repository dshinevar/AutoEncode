...
internal partial class Startup
{
    ...
    static async Task Main()
    {
        ...
        try
        {
            server = container.Resolve<IAutoEncodeServer>();
            server.Initialize();
            await server.Run();  // Runs the server -- OnApplicationExit should initiate shutdown gracefully
        }
        catch (Exception ex)
        {
